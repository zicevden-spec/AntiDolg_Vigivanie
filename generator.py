...
TYPE_INSTRUCTIONS = {
    ...
    "история": "Напиши ОДНУ связную историю (600-900 знаков) про ОДНОГО конкретного человека, который законно избавился от долгов. СТРОГО ЗАПРЕЩЕНО делать список из нескольких людей (Анна, Игорь и т.д.). Текст должен читаться как цельный рассказ, а не как список примеров. Структура: 1) Завязка: имя, возраст, причина долга. 2) Нагнетание: как долги давили (звонки, страх). 3) Перелом: обращение к юристу, осознание, что банкротство — это законное право. 4) Развязка: суд списал долг, жизнь наладилась. Используй эмодзи (🧑‍💼 📉 ⚖️ ✅) как якоря внутри абзацев. Пиши живо, с эмпатией.",
