...
def generate_dzen_article():
    ...
    while attempts < 3:
        ...
        prompt = (
            ...
            f"Пройти опрос: {AFFILIATE_LINK}\n"
            f"Хочу стать агентом: {AGENT_LINK}\n"
            "8. НЕ показывай рассуждения, только готовую статью.\n"
            "9. ВАЖНО: тело статьи строго до 3600 знаков. Законченная мысль важнее длины — никогда не обрывай текст на полуслове.\n"
        )
        for model in WORKING_MODELS:
            try:
                response = client.chat.completions.create(
